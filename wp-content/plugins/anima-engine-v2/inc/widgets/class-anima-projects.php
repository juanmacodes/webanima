...
class Anima_Projects extends Widget_Base {
  public function get_name(){ return 'anima_projects'; }
  public function get_title(){ return __('Anima Proyectos (CPT)','anima-engine'); }
  ...
  protected function render(){
    ...
    $q = new WP_Query(['post_type'=>'proyecto','posts_per_page'=>intval($s['posts_per_page'] ?: 6)]);
    $cols = $s['columns'] ?? '3';
    echo '<div class="anima-grid cols-'.$cols.'">';
    if($q->have_posts()): while($q->have_posts()): $q->the_post();
      $thumb = get_the_post_thumbnail_url(get_the_ID(),'large');
      $client = get_post_meta(get_the_ID(),'anima_client', true);
      $year = get_post_meta(get_the_ID(),'anima_year', true);
      ...
      if($client) echo '<span class="anima-pill" style="background:rgba(91,140,255,.12);color:#bcd1ff">'.esc_html($client).'</span>';
      if($year) echo '<span class="anima-pill" style="background:rgba(49,225,247,.10);color:#9BE7FF">'.intval($year).'</span>';
      echo '</div>';
      echo '<a class="anima-btn anima-btn--ghost" href="'.esc_url(get_permalink()).'">Ver proyecto</a>';
      echo '</div></article>';
    endwhile; wp_reset_postdata(); else: echo '<p>No hay proyectos publicados.</p>'; endif;
    echo '</div>';
  }
}
