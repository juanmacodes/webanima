...
export const metadata: Metadata = {
  ...
  description: 'Formaciones sobre streaming volumétrico, hologramas, IA y VR conectadas a la plataforma Anima.'
};
...
async function CoursesList({ level, modality }: { level?: string; modality?: string }) {
  const courses = await getCourses({ level, modality });

  return (
    <div className="mt-10 grid gap-6 md:grid-cols-2">
      {courses.map((course) => (
        <article key={course.id} className="card flex h-full flex-col justify-between bg-background/70">
          <div>
    ...
    <main className="pt-28">
      <section className="mx-auto w-full max-w-5xl px-6">
        <header className="text-center">
          <h1 className="text-4xl font-semibold md:text-6xl">Cursos y workshops</h1>
          <p className="mt-4 text-base text-foreground/70">
            Ajusta nivel y modalidad para consumir el contenido desde WordPress. Los filtros se envían como query params.
          </p>
        </header>
        ...
        <Suspense
          ...
          fallback={<div className="mt-10 text-center text-sm text-foreground/60">Cargando cursos...</div>}
        >
